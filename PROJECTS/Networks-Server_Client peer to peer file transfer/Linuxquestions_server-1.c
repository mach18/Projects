...
void receive_file(char* fr_name)
{
    ...
    {
        bzero(revbuf, LENGTH);
        int fr_block_sz = 0, file_size = 0;
        while((fr_block_sz = recv(nsockfd, revbuf, LENGTH, 0)) > 0)
        {
            printf("while loop beginning fr_block_sz = %d\n", fr_block_sz);
            file_size++;
            ...
            bzero(revbuf, LENGTH);
            printf("Passed bzero mark\n");
            if (fr_block_sz == 0/* || fr_block_sz != 512*/)
            {
                printf("Breaking the loop due to fr_block_sz = %d\n", fr_block_sz);
                break;
            }
//            else if (fr_block_sz)
//            {
